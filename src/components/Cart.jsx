...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchImagesAndUpdateCart = async () => {
      console.log("Cart", cart);
      try {
        const response = await axios.get("https://ecom-proj-production.up.railway.app/api/products");
        const backendProductIds = response.data.map((product) => product.id);
        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              const response = await axios.get(
                `https://ecom-proj-production.up.railway.app/api/product/${item.id}/image`,
                { responseType: "blob" }
              );
              const imageFile = await converUrlToFile(response.data, response.data.imageName);
              setCartImage(imageFile)
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        ...
        await axios
          .put(`https://ecom-proj-production.up.railway.app/api/product/${item.id}`, cartProduct, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          })
